...
import { check_email_Exsitence } from "@/Services/auth"
...
export function UserModal({ isOpen, onClose, onSave, user, isSaving }: UserModalProps) {
  ...
  const debouncedCheckEmail = useCallback(
    debounce(async (email: string) => {
      ...
      try {
        const response = await check_email_Exsitence(email)
        const exists = response.exists
        if (exists && (!user || user.email !== email)) {
          setEmailExists(true)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (isSaving || emailExists) return
    if (!validate()) return
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              onBlur={() => debouncedCheckEmail.flush()} // immediate check on blur
            />
            {errors.email && <p className="text-sm text-red-600 mt-1">{errors.email}</p>}
          </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              {isSaving ? (
                <div className="flex items-center gap-2">
                  <div className="h-4 w-4 border-2 border-t-transparent border-white animate-spin rounded-full" />
                  Saving...
                </div>
              ) : user ? "Update User" : "Create User"}
            </Button>
